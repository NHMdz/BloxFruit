for i, v in pairs(game.CoreGui:GetChildren()) do
	if v:IsA("ScreenGui") and v.Name == "ToogleUiKhanh" then
		v:Destroy()
	end
end
...
ImageButton.Position = UDim2.new(0.136699513, 0, 0.338056684, 0)
ImageButton.Size = UDim2.new(0, 60, 0, 60)
ImageButton.Image = "http://www.roblox.com/asset/?id=17634661767"
ImageButton.Draggable = true

local a = game:GetService("TweenService")
local b = TweenInfo.new(5, Enum.EasingStyle.Linear, Enum.EasingDirection.InOut, -1)
local c = a:Create(ImageButton, b, {Rotation = 360})
...
do
    ...
    function UpdatePlayerChaMobName()
                ...
                if not isnil(v.Character) then
                    if ESPPlayer then
                        if not isnil(v.Character.Head) and not v.Character.Head:FindFirstChild('NameEsp' .. Number) then
                            ...
                            bill.AlwaysOnTop = true
                            local name = Instance.new('TextLabel', bill)
                            name.Font = Enum.Font.GothaMobNameemibold
                            name.FontSize = "Size14"
                            name.TextWrapped = true
                            name.Text = (v.Name .. ' \n' ..
                                            round(
    ...
    function UpdateChestChaMobName()
    ...
    function UpdateDevilChaMobName()
        for i, v in pairs(game.Workspace:GetChildren()) do
            pcall(function()
    ...
    function UpdateFlowerChaMobName()
        for i, v in pairs(game.Workspace:GetChildren()) do
            pcall(function()
    ...
    function UpdateRealFruitChaMobName()
        ...
        while task.wait() do
            pcall(function()
                if _G.TeleportIsland or _G.AutoQuestRace or _G.AutoBuyBoat or _G.dao or _G.AutoMirage or AutoFarmAcient or
                    ...
                    AutoFarmNoQuest or _G.AutoBone or AutoFarMobNameelectMonsterQuest or AutoFarMobNameelectMonsterNoQuest or
                    _G.AutoBoss or AutoFarmBossQuest or AutoFarmMasGun or AutoFarmMasDevilFruit or AutoFarMobNameelectArea or
                    ...
                    AutoEvoRace or _G.Ecto or _G.killplayersautrial then
    ...
    --------------------------------------------------------------------------------------------------------------------------------------------
    local Section = Tabs.Levia:AddSection("Leviathan")

    Tabs.Levia:AddParagraph({
        Title = "Speed Boats"
    })
    local Togglespeedthuyen = Tabs.Levia:AddToggle("MyTogglespeedthuyen", {
        Title = "Buff Speed Boats",
        Description = "Max: 300",
        Default = false,
        Callback = function(V)
            _G.speedchocaiconcac = V
            if V == false then
    ...
    local Toggleflyboat = Tabs.Levia:AddToggle("MyToggleflyboat", {
        Title = "Fly Boats",
        Default = false,
        Callback = function(V)
            _G.flycaibuoi = V
    ...
    function TweenShip(CFgo)
        local tween_s = game:service "TweenService"
        for i, v in pairs(game:GetService("Workspace").Boats:GetChildren()) do
            if (v.VehicleSeat.CFrame.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame.Position).Magnitude <=
                5 then
                local info = TweenInfo.new((v.VehicleSeat.CFrame.Position - CFgo.Position).Magnitude / 300,
                    Enum.EasingStyle.Linear)
                tween = tween_s:Create(v.VehicleSeat, info, {
                    CFrame = CFgo
                })
            end
        end
    ...
    local Toggletimconlevia = Tabs.Levia:AddToggle("MyToggletimconlevia", {
        Title = "Auto Frozen",
        Description = "Auto find leviathan",
        Default = false,
        Callback = function(V)
            _G.tweenboatscaidaubuoi = V
            if V == false then
                for i, v in pairs(game:GetService("Workspace").Boats:GetChildren()) do
                    if (v.VehicleSeat.CFrame.Position -
                        game.Players.LocalPlayer.Character.HumanoidRootPart.CFrame.Position).Magnitude <= 5 then
...
spawn(function()
    while wait(2) do
        ...
        if ChestESP then
            UpdateChestChaMobName()
        end
        if ESPPlayer then
            UpdatePlayerChaMobName()
        end
        if RealFruitESP then
            UpdateRealFruitChaMobName()
        end
    end
end)
...
local Togglekillplayer = Tabs.Race:AddToggle("Togglekillplayeraftertrial", {
    Title = "Auto Kill Player",
    Description = "After Trial",
    Default = false
})
Togglekillplayer:OnChanged(function(V)
    _G.killplayersautrial = V
end)

spawn(function()
    while wait() do
        pcall(function()
            if _G.killplayersautrial then
                    ...
                    if v.Name ~= game.Players.LocalPlayer.Name and
                        ...
                        if v.Humanoid.Health >= 0 then
                            repeat
                                ...
                                EquipTool(SelectWeapon)
                                Click()
                            until not _G.killplayersautrial or not v.Parent or v.Humanoid.Health <= 0
                        end
                    end
                end
            end
...
local Toggleusekillplayer = Tabs.Race:AddToggle("Togglekillplayeraftertrial", {
    Title = "Auto use skill",
    Description = "Using with Kill Players",
    Default = false
})
Toggleusekillplayer:OnChanged(function(V)
    _G.skillkillplayersautrial = V
end)

spawn(function()
    while wait() do
        pcall(function()
            if _G.skillkillplayersautrial and _G.killplayersautrial then
                for i, v in pairs((game:GetService("Workspace")).Characters:GetChildren()) do
                    if v.Name ~= game.Players.LocalPlayer.Name and
                        (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <=
                        10 or v.Name ~= _G.PlayerSelected and _G.khongkillngchoi and
                        (v.HumanoidRootPart.Position - game.Players.LocalPlayer.Character.HumanoidRootPart.Position).Magnitude <=
                        10 then
                        if v.Humanoid.Health > 0 then
                            repeat
                            ...
                            until not _G.skillkillplayersautrial or (not v.Parent) or v.Humanoid.Health <= 0
                        end
                    end
                end
            end
...
local DropdownPlayerlisttrial = Tabs.Race:AddDropdown("Playerlisttrial", {
    Title = "Players List",
    Values = playerNames,
    Multi = false,
    Default = 1
...
DropdownPlayerlisttrial:OnChanged(function(Value)
    _G.PlayerSelected = Value
end)

local Toggleusekillplayer = Tabs.Race:AddToggle("Togglekillplayeraftertrial", {
    Title = "Don't Kill Player Selected",
    Default = false
})
Toggleusekillplayer:OnChanged(function(V)
    _G.khongkillngchoi = V
end)
--------------------------------------------------------------------------------------------------------------------------------------------
-- shop
